...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$ScriptDir\Backend Money Card'; npm run dev"

# 2. Start Prisma Studio (Port 5555)
Write-Host "[2/4] Starting Prisma Studio on http://localhost:5555..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$ScriptDir\Backend Money Card'; npx prisma studio"

# 3. Start Frontend Web Admin (Port 5173)
Write-Host "[3/4] Starting Web Admin on http://localhost:5173..." -ForegroundColor Green
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$ScriptDir\Frontend Money Card'; npm run dev"
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$ScriptDir\Flutter Money card'; flutter run -d chrome --web-port=5000"
